...
internal sealed class EndpointReviewForm : Form
{
    ...
    private readonly Button _start = Button(AppLocalization.T("Endpoint.Form.Start"), "EndpointStart");
    private readonly Button _stop = Button(AppLocalization.T("Endpoint.Form.Stop"), "EndpointStop");
    private readonly Button _copy = Button(AppLocalization.T("Endpoint.Form.Copy"), "EndpointCopy");
    private readonly Button _export = Button(AppLocalization.T("Endpoint.Form.Export"), "EndpointExport");
    private readonly Button _close = Button(AppLocalization.T("Endpoint.Form.Close"), "EndpointClose");
    private readonly TextBox _search = new() { Name = "EndpointSearch", Width = 350, PlaceholderText = AppLocalization.T("Endpoint.Form.Search") };
    ...
    private string _stage = AppLocalization.T("Endpoint.Form.Ready");

    public EndpointReviewForm()
    {
        Name = "EndpointReview"; Text = AppLocalization.T("Endpoint.Form.Title");
        ...
        Controls.Add(root);
        root.Controls.Add(new Label { Dock = DockStyle.Fill, AutoSize = true, Padding = new Padding(0, 0, 0, 8), Text = AppLocalization.T("Endpoint.Form.Intro") }, 0, 0);
        ...
        _filter.Items.AddRange([
            AppLocalization.T("Endpoint.Filter.All"), AppLocalization.T("Endpoint.Filter.Tcp"), AppLocalization.T("Endpoint.Filter.Udp"),
            AppLocalization.T("Endpoint.Filter.Listen"), AppLocalization.T("Endpoint.Filter.Established"), AppLocalization.T("Endpoint.Filter.Changes")
        ...
        Column("Change", AppLocalization.T("Endpoint.Column.Change"), typeof(string), 190); Column("Table", AppLocalization.T("Endpoint.Column.Protocol"), typeof(string), 70); Column("PID", "PID", typeof(uint), 70);
        Column("Process", AppLocalization.T("Endpoint.Column.Process"), typeof(string), 160); Column("Local", AppLocalization.T("Endpoint.Column.LocalIp"), typeof(string), 170); Column("LocalPort", AppLocalization.T("Endpoint.Column.Port"), typeof(int), 65);
        Column("Remote", AppLocalization.T("Endpoint.Column.RemoteIp"), typeof(string), 170); Column("RemotePort", AppLocalization.T("Endpoint.Column.Port"), typeof(int), 65); Column("State", AppLocalization.T("Endpoint.Column.State"), typeof(string), 130);
        _grid.Columns["Process"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
        _grid.Columns["Process"].MinimumWidth = 140;
        _overview.Text = AppLocalization.T("Endpoint.Form.OverviewEmpty", AppLocalization.T("Endpoint.Form.Start"));
        _detail.Text = AppLocalization.T("Endpoint.Form.DetailEmpty");
        _start.Click += async (_, _) => await CollectAsync();
        _stop.Click += (_, _) => { _cancellation?.Cancel(); _stage = AppLocalization.T("Endpoint.Form.StopRequested"); UpdateButtons(); };
        ...
        _timer.Tick += (_, _) => { if (_busy) _status.Text = AppLocalization.T("Endpoint.Form.Elapsed", _stage, _elapsed.Elapsed.TotalSeconds); };
        FormClosing += (_, e) => { if (_exporting) { e.Cancel = true; _stage = AppLocalization.T("Endpoint.Form.WaitSave"); } else _cancellation?.Cancel(); };
        AcceptButton = _start; CancelButton = _close; _status.Text = _stage; UpdateButtons();
    }
    private async Task CollectAsync()
    {
        if (_busy) return;
        using var cancellation = new CancellationTokenSource(); _cancellation = cancellation; _busy = true; _stage = AppLocalization.T("Endpoint.Form.WaitCollector"); _elapsed.Restart(); _timer.Start(); UpdateButtons();
        try
        {
        ...
        catch (OperationCanceledException) { if (!IsDisposed) _status.Text = AppLocalization.T("Endpoint.Form.Cancelled"); }
        catch (Exception ex) { if (!IsDisposed) _status.Text = AppLocalization.T("Endpoint.Form.CollectionFailed", EndpointReviewService.Describe(ex)); }
    ...
    private void RenderRows()
    {
        ...
        _status.Text = _current is null ? AppLocalization.T("Endpoint.Form.NotCollected") : AppLocalization.T("Endpoint.Form.Status", EndpointReviewCore.CollectionText(_current.State), _current.FinishedAt, matches.Count, _grid.Rows.Count);
    }
    private async Task ExportAsync()
    {
        if (_busy || _current is not { } current) return;
        using var dialog = new FolderBrowserDialog { Description = AppLocalization.T("Endpoint.Form.ExportDescription"), UseDescriptionForTitle = true };
        if (dialog.ShowDialog(this) != DialogResult.OK) return;
        var previous = _previous; var target = dialog.SelectedPath;
        _busy = _exporting = true; _stage = AppLocalization.T("Endpoint.Form.Saving"); _elapsed.Restart(); _timer.Start(); UpdateButtons();
        try { var folder = await Task.Run(() => EndpointReviewReport.Save(current, previous, target)); if (!IsDisposed) _status.Text = AppLocalization.T("Endpoint.Form.Saved", folder); }
        catch (Exception ex)
        {
            if (!IsDisposed)
            {
                ApplyExportFailureStatus(ex);
                MessageBox.Show(this, AppLocalization.T("Endpoint.Form.SaveFailedMessage", ex.Message), AppLocalization.T("Endpoint.Form.ExportTitle"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    ...
    {
        _status.Text = AppLocalization.T("Endpoint.Form.SaveFailed", ex.GetType().Name, ex.HResult.ToString("X8"));
    }
    ...
    private void TryUi(Action action) { try { action(); } catch (Exception ex) { MessageBox.Show(this, ex.Message, AppLocalization.T("Endpoint.Form.ActionFailed"), MessageBoxButtons.OK, MessageBoxIcon.Warning); } }
...
internal static class EndpointReviewMenu
{
    public static void Attach(Form main)
    {
        ...
        var item = new ToolStripMenuItem(AppLocalization.T("Endpoint.Menu.Item")) { Name = "EndpointReviewOpen" };
