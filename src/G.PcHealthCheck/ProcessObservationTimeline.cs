...
internal sealed class ProcessObservationTimeline : Control
{
    ...
    {
        Name = "ProcessTimeline"; AccessibleName = "График выбранного процесса";
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        if (s is null) { e.Graphics.DrawString("Наблюдение за процессом ещё не начато.", Font, Brushes.DimGray, 14, 24); return; }
        ...
        for (var i = 0; i <= 4; i++) { var value = ymax * i / 4; var y = Y(value); g.DrawLine(grid, bounds.Left, y, bounds.Right, y); g.DrawString(value.ToString("0.##"), Font, Brushes.DimGray, 3, y - Font.Height / 2f); }
        ...
        if (segments.Count == 0) g.DrawString("Нет доступных значений — это не нулевая нагрузка.", Font, Brushes.DimGray, bounds.Left + 10, bounds.Top + 20);
        g.DrawString("0 с", Font, Brushes.DimGray, bounds.Left, bounds.Bottom + 8);
        var end = $"{xmax / 1000:0.0} с"; g.DrawString(end, Font, Brushes.DimGray, bounds.Right - g.MeasureString(end, Font).Width, bounds.Bottom + 8);
    }
}
