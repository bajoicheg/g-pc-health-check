...
internal sealed class FileUseForm : Form
{
    ...
    private readonly TextBox _target = new() { Name = "FileUseTarget", Dock = DockStyle.Fill, PlaceholderText = AppLocalization.T("FileUse.Form.TargetPlaceholder") };
    private readonly TextBox _search = new() { Name = "FileUseSearch", Width = 430, PlaceholderText = AppLocalization.T("FileUse.Form.SearchPlaceholder") };
    private readonly Button _browse = Button(AppLocalization.T("FileUse.Form.Browse"), "FileUseBrowse");
    private readonly Button _start = Button(AppLocalization.T("FileUse.Form.Start"), "FileUseStart");
    private readonly Button _stop = Button(AppLocalization.T("FileUse.Form.Stop"), "FileUseStop");
    private readonly Button _copy = Button(AppLocalization.T("FileUse.Form.Copy"), "FileUseCopy");
    private readonly Button _export = Button(AppLocalization.T("FileUse.Form.Export"), "FileUseExport");
    private readonly Button _close = Button(AppLocalization.T("FileUse.Form.Close"), "FileUseClose");
    ...
    private string _stage = AppLocalization.T("FileUse.Form.StageInitial");

    public FileUseForm()
    {
        Name = "FileUseReview"; Text = AppLocalization.T("FileUse.Form.Title");
        ...
        Controls.Add(root);
        root.Controls.Add(new Label { Dock = DockStyle.Fill, AutoSize = true, Padding = new Padding(0, 0, 0, 8), Text = AppLocalization.T("FileUse.Form.Intro") }, 0, 0);
        ...
        Column("Pid", "PID", typeof(uint), 80); Column("Application", AppLocalization.T("FileUse.Column.Application"), typeof(string), 200); Column("Service", AppLocalization.T("FileUse.Column.Service"), typeof(string), 155);
        Column("Type", AppLocalization.T("FileUse.Column.Type"), typeof(string), 140); Column("Session", AppLocalization.T("FileUse.Column.Session"), typeof(uint), 85); Column("Exe", AppLocalization.T("FileUse.Column.Exe"), typeof(string), 190); Column("Identity", AppLocalization.T("FileUse.Column.Identity"), typeof(string), 230);
        _grid.Columns["Application"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill; _grid.Columns["Application"].MinimumWidth = 170;
        _overview.Text = AppLocalization.T("FileUse.Form.Overview");
        ...
        _stop.Click += (_, _) => { _cancellation?.Cancel(); _stage = AppLocalization.T("FileUse.Form.StopRequested"); UpdateButtons(); };
        ...
        _timer.Tick += (_, _) => { if (_busy) _status.Text = AppLocalization.T("FileUse.Form.Elapsed", _stage, _elapsed.Elapsed.TotalSeconds); };
        FormClosing += (_, e) => { if (_exporting) { e.Cancel = true; _stage = AppLocalization.T("FileUse.Form.WaitSave"); } else _cancellation?.Cancel(); };
        AcceptButton = _start; CancelButton = _close; UpdateButtons();
    }
    private void Browse()
    {
        using var dialog = new OpenFileDialog { Title = AppLocalization.T("FileUse.Form.BrowseTitle"), CheckFileExists = true, Multiselect = false, Filter = AppLocalization.T("FileUse.Form.BrowseFilter"), DereferenceLinks = false };
    ...
    {
        var summary = AppLocalization.T("FileUse.Form.Status", FileUseCore.StateText(snapshot.State), snapshot.FinishedAt, visibleCount, snapshot.Processes.Count);
        _status.Text = TargetChanged(snapshot)
            ? AppLocalization.T("FileUse.Form.TargetChanged", summary)
            : summary;
    }
    private async Task CollectAsync()
    {
        ...
        catch (ArgumentException ex) { MessageBox.Show(this, ex.Message, AppLocalization.T("FileUse.Form.PathTitle"), MessageBoxButtons.OK, MessageBoxIcon.Information); return; }
        using var cancellation = new CancellationTokenSource(); _cancellation = cancellation; _busy = true; _stage = AppLocalization.T("FileUse.Form.WaitCollector"); _elapsed.Restart(); _timer.Start(); UpdateButtons();
        try
        {
        ...
        catch (OperationCanceledException) { if (!IsDisposed) _status.Text = AppLocalization.T("FileUse.Form.Cancelled"); }
        catch (Exception ex) { if (!IsDisposed) _status.Text = AppLocalization.T("FileUse.Form.Failed", ex.GetType().Name, ex.Message); }
    ...
    private async Task ExportAsync()
    {
        ...
        using var dialog = new FolderBrowserDialog { Description = AppLocalization.T("FileUse.Form.ExportDescription"), UseDescriptionForTitle = true };
        if (dialog.ShowDialog(this) != DialogResult.OK) return;
        var previous = _previous; var parent = dialog.SelectedPath;
        _busy = _exporting = true; _stage = AppLocalization.T("FileUse.Form.Saving"); _elapsed.Restart(); _timer.Start(); UpdateButtons();
        try { var folder = await Task.Run(() => FileUseReport.Save(current, previous, parent)); if (!IsDisposed) _status.Text = AppLocalization.T("FileUse.Form.Saved", folder); }
        catch (Exception ex)
        {
            if (!IsDisposed)
            {
                ApplyExportFailure(ex);
                MessageBox.Show(this, AppLocalization.T("FileUse.Form.SaveFailedMessage", ex.Message), AppLocalization.T("FileUse.Form.ExportTitle"), MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
    ...
    {
        _status.Text = AppLocalization.T("FileUse.Form.SaveFailed", ex.GetType().Name, ex.HResult.ToString("X8"));
    }
    ...
    private void TryUi(Action action) { try { action(); } catch (Exception ex) { MessageBox.Show(this, ex.Message, AppLocalization.T("FileUse.Form.ActionFailed"), MessageBoxButtons.OK, MessageBoxIcon.Warning); } }
...
internal static class FileUseMenu
{
    public static void Attach(Form main)
    {
        ...
        var item = new ToolStripMenuItem(AppLocalization.T("FileUse.Menu.Item")) { Name = "FileUseOpen" };
